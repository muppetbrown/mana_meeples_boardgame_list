...
class PerformanceMonitor:
    ...
    def __init__(self):
        ...
        self.endpoint_stats = (
            OrderedDict()
        )  # Use OrderedDict to prevent unbounded growth
    ...
    def record_request(
        ...
        if (
            endpoint_key not in self.endpoint_stats
            and len(self.endpoint_stats) >= self.max_endpoints
        ):
                ...
                [
                    {
                        "endpoint": endpoint,
                        "avg_time_ms": round(
                            (stats["total_time"] / stats["count"]) * 1000, 2
                        ),
