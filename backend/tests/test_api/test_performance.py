...
@pytest.fixture
def large_game_dataset(test_db):
...
class TestAPIPerformance:
    ...
    def test_game_list_endpoint_performance(self, test_client, large_game_dataset):
    ...
    def test_game_detail_endpoint_performance(self, test_client, large_game_dataset):
    ...
    def test_search_performance(self, test_client, large_game_dataset):
        """Search should complete within 300ms"""
        start = time.time()
        response = test_client.get('/api/public/games?q=test&page_size=24')
        duration = time.time() - start

        assert response.status_code == 200
        assert duration < 0.3, f"Search took {duration:.3f}s, expected <0.3s"

    def test_category_filter_performance(self, test_client, large_game_dataset):
        """Category filtering should complete within 150ms"""
        start = time.time()
        response = test_client.get('/api/public/games?category=GATEWAY_STRATEGY&page_size=24')
        duration = time.time() - start

        assert response.status_code == 200
        assert duration < 0.15, f"Category filter took {duration:.3f}s, expected <0.15s"

    def test_complex_filter_performance(self, test_client, large_game_dataset):
    ...
    def test_category_counts_performance(self, test_client, large_game_dataset):
    ...
    def test_pagination_performance(self, test_client, large_game_dataset):
    ...
    def test_large_page_size_performance(self, test_client, large_game_dataset):
    ...
    def test_sort_performance(self, test_client, large_game_dataset):
        """Sorting should not significantly impact performance"""
        sorts = ['title_asc', 'year_desc', 'rating_desc']

        for sort in sorts:
            start = time.time()
            response = test_client.get(f'/api/public/games?sort={sort}&page_size=24')
            duration = time.time() - start

            assert response.status_code == 200
            assert duration < 0.25, f"Sort {sort} took {duration:.3f}s, expected <0.25s"

    def test_nz_designer_filter_performance(self, test_client, large_game_dataset):
        """NZ designer filtering should be efficient"""
        start = time.time()
        response = test_client.get('/api/public/games?nz_designer=true&page_size=24')
        duration = time.time() - start

        assert response.status_code == 200
        assert duration < 0.15, f"NZ designer filter took {duration:.3f}s, expected <0.15s"

    def test_designer_search_performance(self, test_client, large_game_dataset):
        """Designer search should complete within 300ms"""
        start = time.time()
        response = test_client.get('/api/public/games?designer=Designer 1')
        duration = time.time() - start

        assert response.status_code == 200
        assert duration < 0.3, f"Designer search took {duration:.3f}s, expected <0.3s"

    def test_rapid_sequential_requests(self, test_client, large_game_dataset):
    ...
    def test_database_health_check_performance(self, test_client, large_game_dataset):
    ...
    def test_admin_game_list_performance(self, test_client, large_game_dataset):
    ...
    def test_concurrent_read_performance(self, test_client, large_game_dataset):
    ...
    def test_response_size_efficiency(self, test_client, large_game_dataset):
    ...
    def test_empty_result_performance(self, test_client, large_game_dataset):
        """Empty results should return quickly"""
        start = time.time()
        response = test_client.get('/api/public/games?q=nonexistentgamexyz12345')
        duration = time.time() - start

        assert response.status_code == 200
        assert duration < 0.1, f"Empty result took {duration:.3f}s, expected <0.1s"

    def test_database_query_count(self, test_client, large_game_dataset):
    ...
    def test_memory_efficiency(self, test_client, large_game_dataset):
