...
class TestConcurrentLoad:
    ...
    def test_concurrent_game_list_requests(self, test_client, large_game_dataset):
    ...
    def test_concurrent_search_requests(self, test_client, large_game_dataset):
        ...
        def search_games(term):
            try:
                response = test_client.get(f'/api/public/games?q={term}')
    ...
    def test_concurrent_filter_requests(self, test_client, large_game_dataset):
        ...
        def filter_games(category):
            response = test_client.get(f'/api/public/games?category={category}')
    ...
    def test_mixed_read_workload(self, test_client, large_game_dataset):
        """Should handle mixed read operations concurrently"""
        operations = [
            lambda: test_client.get('/api/public/games?page_size=24'),
            lambda: test_client.get('/api/public/games?q=test'),
            lambda: test_client.get('/api/public/games?category=GATEWAY_STRATEGY'),
            lambda: test_client.get('/api/public/category-counts'),
            lambda: test_client.get(f'/api/public/games/{large_game_dataset[0].id}'),
        ]
    ...
    def test_sustained_load(self, test_client, large_game_dataset):
    ...
    def test_concurrent_admin_operations(self, test_client, large_game_dataset):
    ...
    def test_rate_limiting_under_load(self, test_client):
    ...
    def test_database_connection_pool(self, test_client, large_game_dataset):
        """Should manage database connections efficiently"""
        errors = []

        def query_database():
            try:
                response = test_client.get('/api/public/games?page_size=10')
                if response.status_code != 200:
    ...
    def test_concurrent_pagination(self, test_client, large_game_dataset):
        """Should handle concurrent pagination requests"""
        pages = list(range(1, 11))  # Pages 1-10
        results = []

        def fetch_page(page):
            response = test_client.get(f'/api/public/games?page={page}&page_size=24')
            return response.status_code == 200
    ...
    def test_concurrent_sorting(self, test_client, large_game_dataset):
        ...
        def sort_games(sort_by):
            response = test_client.get(f'/api/public/games?sort={sort_by}&page_size=24')
            return response.status_code == 200
    ...
    def test_peak_load_simulation(self, test_client, large_game_dataset):
        ...
        def simulate_user():
            try:
                ...
                test_client.get('/api/public/games?page_size=24')
                test_client.get('/api/public/games?q=game')
                test_client.get(f'/api/public/games/{large_game_dataset[0].id}')
                return True
            except Exception:
                return False
    ...
    def test_error_recovery_under_load(self, test_client, large_game_dataset):
        ...
        def make_mixed_requests():
            try:
                ...
                valid = test_client.get('/api/public/games?page_size=24')
                invalid = test_client.get('/api/public/games/999999')  # Non-existent
    ...
    def test_cache_effectiveness_under_load(self, test_client, large_game_dataset):
        """Should benefit from caching under repeated requests"""
        # First request (cache miss)
        start = time.time()
        test_client.get('/api/public/category-counts')
    ...
    def test_connection_timeout_handling(self, test_client):
    ...
    def test_memory_stability_under_load(self, test_client, large_game_dataset):
    ...
    def test_graceful_degradation(self, test_client, large_game_dataset):
