...
export default function GameImage({ url, alt, className, fallbackClass, loading = "lazy" }) {
  ...
  if (!url || imageError) {
    return (
      <div className={fallbackClass || "w-16 h-16 sm:w-20 sm:h-20 bg-slate-200 rounded-xl flex flex-col items-center justify-center text-slate-700 text-xs sm:text-sm transition-colors hover:bg-slate-300"}>
