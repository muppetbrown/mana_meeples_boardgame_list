...
export function ManualGameEntryPanel({ onSuccess, onToast }) {
  ...
  const validateAndPrepare = () => {
    ...
    // Validate BGG ID if provided and valid (must be positive integer)
    ...
    // Validate year if provided and valid (reasonable range)
    ...
    // Validate player counts (only when valid numbers are provided)
    ...
    // Validate playtime (only when valid numbers are provided)
    ...
    // Validate rating (0-10) - only when valid number is provided
    const rating = parseFloatValue(formData.average_rating);
    if (rating !== null && (rating < 0 || rating > 10)) {
      errors.push("BGG Rating must be between 0 and 10");
    }

    // Validate complexity (1-5) - only when valid number is provided
