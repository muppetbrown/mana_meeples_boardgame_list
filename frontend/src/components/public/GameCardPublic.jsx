...
export default function GameCardPublic({ game, lazy = false, onShare }) {
  ...
  return (
    <article className="group bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-2xl border-2 border-slate-200 transition-all duration-300 hover:scale-[1.02] hover:border-emerald-300 focus-within:ring-4 focus-within:ring-emerald-200 focus-within:ring-offset-2">
      {/* Main Link */}
      <Link
        to={href}
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200 aspect-square">
          ...
          {categoryLabel && (
            <div className="absolute top-3 right-3">
              <span 
                className={`px-3 py-1.5 rounded-lg text-xs font-bold shadow-lg border-2 backdrop-blur-sm ${getCategoryStyle(game.mana_meeple_category)}`}
          ...
          {formatRating(game.average_rating) && (
            <div className="absolute top-3 left-3">
              <div className="bg-white/95 backdrop-blur-sm border-2 border-yellow-400 rounded-lg px-2.5 py-1.5 shadow-lg">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-sm font-bold text-slate-800">{formatRating(game.average_rating)}</span>
                </div>
              </div>
            </div>
          )}
      ...
      {onShare && (
        <div className="absolute top-3 right-3 z-10">
          <button
            ...
            className="
              w-8 h-8 rounded-full bg-white/90 backdrop-blur-sm border-2 border-slate-300 
